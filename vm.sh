...
then
		  ...
		  VBoxManage createhd --filename `pwd`/sgoinfre/$(whoami)/$MACHINENAME/$MACHINENAME_DISK.vdi --size 10000 --format VDI
fi
...
then
		  ...
		  VBoxManage storageattach $MACHINENAME --storagectl "SATA Controller" --port 0 --device 0 --type hdd --medium  `pwd`/sgoinfre/$(whoami)/$MACHINENAME/$MACHINENAME_DISK.vdi
fi
